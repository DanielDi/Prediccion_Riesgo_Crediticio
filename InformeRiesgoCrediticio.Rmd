...
bins <- woebin(df_num, y = "good_bad", method = "tree")
...
```{r, include=FALSE}
woe_model <- woe(as.factor(train$good_bad) ~ ., data = train)
woe_model$xnew

train_woes <- data.frame(good_bad = train$good_bad, 
                         woe_model$xnew)

valid_woes <- predict(woe_model, subset(valid, select = c(-good_bad)))
valid_woes
# model <- glm(good_bad ~ ., data = train_woes, family = binomial)
summary(model)
```
